...
def run_client(value):
    p = subprocess.Popen(["node", f"{HERE}/../../../src/__tests__/clients/test/ynotebook-update-source.mjs"])
    return 0


@pytest.mark.asyncio
async def test_notebook_update_source(y_websocket_server):
    ynotebook = YNotebook()
    nb = stringify_source(json.loads((NOTEBOOKS_DIR / "simple.ipynb").read_text()))
    ynotebook.source = nb
    async with connect("ws://127.0.0.1:1234/update_source") as websocket, WebsocketProvider(ynotebook.ydoc, websocket):
