...
describe('Y.js Standalone', () => {
  ...
  it('y.js mutations', async () => {
    ...
    const yarray = ydoc.getArray('my array');
    try {
      const ymap = ydoc.getMap('favorites') as Map<string>;
...
describe('Y.js Standalone 2', () => {
  ...
  it('y.js standalone', async () => {
    ...
    const stateVector1 = encodeStateVector(doc1);
    const stateVector2 = encodeStateVector(doc2);
    const diff1 = encodeStateAsUpdate(doc1, stateVector2);
    const diff2 = encodeStateAsUpdate(doc2, stateVector1);
    ...
    const relPos1 = createRelativePositionFromTypeIndex(ytext, 2);
    const pos1 = createAbsolutePositionFromRelativePosition(relPos1, doc);
    pos1?.type === ytext; // => true
    pos1?.index === 2; // => true

    // Send relative position to remote client (json).
    const relPos = createRelativePositionFromTypeIndex(ytext, 2);
    const encodedRelPos = JSON.stringify(relPos);

    // send encodedRelPos to remote client.
    const parsedRelPos = JSON.parse(encodedRelPos);
    const pos = createAbsolutePositionFromRelativePosition(parsedRelPos, doc);
    pos?.type === ytext; // => true
    pos?.index === 2; // => true
    /*
    // Send relative position to remote client (Uint8Array)
    const relPos2 = createRelativePositionFromTypeIndex(ytext, 2)
    const encodedRelPos2 = encodeRelativePosition(relPos)
    // send encodedRelPos to remote client..
    const parsedRelPos2 = Y.decodeRelativePosition(encodedRelPos)
    const pos2 = createAbsolutePositionFromRelativePosition(parsedRelPos, remoteDoc)
    ...
    */
    ...
    const undoManager = new UndoManager(ytext1);
    ytext1.insert(0, 'abc');
    undoManager.undo();
    ytext1.toString(); // => ''
    undoManager.redo();
    ...
    const undoManager2 = new UndoManager(ytext2, {
    ...
    /*
    const undoManager3 = new UndoManager(ytext3, {
      trackedOrigins: new Set([42, CustomBinding])
    })
    */
    /*
